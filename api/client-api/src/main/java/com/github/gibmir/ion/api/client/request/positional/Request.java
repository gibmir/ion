...
import com.github.gibmir.ion.api.client.builder.JsonRpcRequestBuilder;
...
public class Request<R> implements JsonRpcClient<R>, JsonRpcRequestBuilder<R> {
  ...
  public Request(String procedureName, Class<R> resultType, JsonRpcRequestIdSupplier jsonRpcRequestIdSupplier,
  ...
  @Override
  public Request jsonb(Jsonb jsonb) {
    this.jsonb = jsonb;
    return this;
  }
  ...
  public CompletableFuture<R> execute() {
    ...
    JsonRpcRequest request = new RequestDto(id, methodName, args);
    byte[] requestPayload = jsonb.toJson(request).getBytes(charset);
    RequestContext requestContext = RequestContext.defaultContext(methodName, requestPayload, id, timeout);
